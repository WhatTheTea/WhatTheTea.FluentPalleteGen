...
namespace WhatTheTea.FluentPalleteGen.Internal
{
    ...
    internal static class ColorBlending
    {
        ...
        public static ARGB ComputeAlphaBlend(ARGB foreground, ARGB background)
        {
            if (foreground.A == 255)
            {
                return foreground;
            ...
            if (foreground.A == 0)
            {
                return ARGB.FromArgb(255, background.R, background.G, background.B);
            }
            ...
            return ARGB.FromArgb(255, (byte)Math.Round(255.0 * finalr), (byte)Math.Round(255.0 * finalg), (byte)Math.Round(255.0 * finalb));
        }
    }
}
